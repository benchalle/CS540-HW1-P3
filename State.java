...
public class State {
	...
	public ArrayList<State> getSuccessors(boolean[][] explored, Maze maze) {
		...
		int currentY = getY();
		 //check left
		if(!explored[currentX][currentY-1] && maze.getSquareValue(currentX, currentY-1) != '%'){
				states.add(new State(new Square(currentX,currentY-1),
						this,this.gValue+1, this.depth+1));
		}
		//check down
		if(!explored[currentX+1][currentY] && maze.getSquareValue(currentX+1, currentY )!= '%'){
				states.add(new State(new Square(currentX+1,currentY),
						this,this.gValue+1, this.depth+1));
		}
		//check right
		if(!explored[currentX][currentY+1] && maze.getSquareValue(currentX, currentY+1) != '%'){
